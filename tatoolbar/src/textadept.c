...
// USE_TA_TOOLBAR and UNUSED() changes: Copyright 2016-2020 Gabriel Dubatti. See LICENSE.
#define USE_TA_TOOLBAR
#define UNUSED(expr) do { (void)(expr); } while (0)
#define TA_VERSION 108  //update to textadept 10.8
...
static int lfind_next(lua_State *L) {UNUSED(L); return (f_clicked(fnext_button, NULL), 0);}

/** `find.find_prev()` Lua function. */
static int lfind_prev(lua_State *L) {UNUSED(L); return (f_clicked(fprev_button, NULL), 0);}
...
static int lfind_replace(lua_State *L) {UNUSED(L); return (f_clicked(r_button, NULL), 0);}
...
static int t_tabbuttonpress(GtkWidget *label, GdkEventButton *event, void*__) {
  GtkNotebook *tabs = GTK_NOTEBOOK(tabbar);
...
static int s_buttonpress(GtkWidget*_, GdkEventButton *event, void*__) {
  ...
  if (event->type == GDK_BUTTON_PRESS && event->button == 3)
    return (lL_showcontextmenu(lua, event, "context_menu"), TRUE);
  return FALSE;
}
#endif
...
int WINAPI WinMain(HINSTANCE _, HINSTANCE __, LPSTR ___, int ____) {
  UNUSED(_); UNUSED(__); UNUSED(___); UNUSED(____);
