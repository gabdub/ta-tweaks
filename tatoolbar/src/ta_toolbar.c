  ...
  if( p != ttb.philight ){
    ...
    if( p->back_color == BKCOLOR_PICKER ){
      ...
      if( ttb.philight != NULL ){
        fire_item_event(p, TEV_CLICK, state); //scroll buffer while moving with mouse down
      }
...
{  //generate a click in tab: -1:prev,1:next,0:first,2:last
  ...
  if( G != NULL ){
    ...
    if( (p != NULL) && (p != pcurr) ){
      fire_tab_event(p, TEV_CLICK, 0); //fire "toolbar_tabclicked" EVENT
    }
  }
}
    ...
    for( p= g->list; (p != NULL); p= p->next ){
      ...
      if( (y0+p->bary2 >= 0) && need_redraw( pdrawarea, x0+p->barx1, y0+p->bary1, x0+p->barx2, y0+p->bary2) ){
        ...
        if( (p->flags & TTBF_TEXT) == 0 ){
          ...
          struct toolbar_img * di= get_item_img(p,h,p->imgbase);
          if( (di != NULL) && (di->fname != NULL) ){ //draw normal/disabled background if set
